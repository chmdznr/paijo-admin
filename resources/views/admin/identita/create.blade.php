...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.identitum.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.identita.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('suku') ? 'is-invalid' : '' }}" type="text" name="suku" id="suku" value="{{ old('suku', '') }}">
                @if($errors->has('suku'))
                    <div class="invalid-feedback">
                        {{ $errors->first('suku') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Identitum::RIWAYAT_PEROKOK_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('riwayat_perokok') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="riwayat_perokok_{{ $key }}" name="riwayat_perokok" value="{{ $key }}" {{ old('riwayat_perokok', 'T') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status_pernikahan') ? 'is-invalid' : '' }}" name="status_pernikahan" id="status_pernikahan">
                    ...
                    @foreach(App\Models\Identitum::STATUS_PERNIKAHAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status_pernikahan', 'M') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal_pertama') ? 'is-invalid' : '' }}" type="text" name="tanggal_pertama" id="tanggal_pertama" value="{{ old('tanggal_pertama') }}">
